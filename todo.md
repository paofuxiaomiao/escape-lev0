# Project TODO

- [x] 伪装Cloudflare验证的欢迎页面
- [x] 五层级游戏流程（进门→电梯→签到→涂鸦→室内开发）
- [x] 后室阈限空间视觉风格（扫描线、荧光灯闪烁、glitch效果）
- [x] 响应式设计支持移动端
- [x] LEV0品牌元素融入（logo、文案、access token）
- [x] 数据库schema设计（视频表、层级表、游戏记录表）
- [x] 后台管理界面 - 视频上传功能
- [x] 后台管理界面 - 层级配置管理
- [x] 后台管理界面 - 视频列表管理（增删改查）
- [x] 视频上传API（对接S3存储）
- [x] 游戏前端对接后端API获取视频数据
- [x] 管理员权限控制（仅admin可访问后台）
- [x] 视频完整编辑功能（标题、异常描述、所属层级、启用状态）
- [x] 粒子效果系统（转场爆发粒子、浮动碎片、能量波）
- [x] 全屏转场动画（冲击波、画面撕裂、扫描线加速）
- [x] 更具感染力的字体设计（标题字体、故障文字动画）
- [x] 层级切换时的转场效果集成
- [x] 换用更有冲击力的字体（Audiowide/Share Tech Mono/Noto Sans SC Black+发光特效）
- [x] 创建路程地图页面（参考关系映射视图风格，暗色底+网格+虚线路径）
- [x] 使用实景照片作为每个层级的背景缩略图
- [x] 进入游戏后先展示路程地图，再进入对应层级
- [x] 修复dotenv依赖错误（服务器启动报ERR_MODULE_NOT_FOUND）
- [x] 修复Game.tsx读取localStorage选中层级（从地图选择层级后正确开始游戏）
- [x] 修复LevelMap层级状态逻辑（locked判断方向修正，levelId递减方向）
- [x] 修复LevelMap与Game之间的进度存储格式一致性
- [x] 字体升级为Audiowide+Share Tech Mono+Rajdhani组合，添加发光/色差/描边特效
- [x] 全局BGM播放器（循环播放The_False_Wall.mp3，带静音/音量控制按钮）
- [x] Level 5视频配置（原视频=正常，修改后视频=异常）已上传并写入数据库
- [x] 修正路程地图图片顺序（LEV5进门=大厅红气球，LEV1室内开发=红门黄衣人）
- [x] 添加Level 0最终挑战层级（数据库+前端游戏逻辑+地图）
- [x] Level 0视频配置（1原视频+4修改后视频）
- [x] 修改游戏逻辑：每关必须展示异常视频，只有“发现异常”才能过关，“未发现异常”则重来
- [x] Level 3视频配置（原视频=正常，修改后视频=异常）
- [x] 替换涂鸦层级图片为新的涂鸦墙照片
- [x] 放大地图节点图片尺寸，能看清细节
- [x] 生成复古氛围感地图背景
- [x] 图片展示改为墙洞效果（从破旧墙壁洞口窥视）
- [x] 游戏页面添加随时返回地图按钮
- [x] 全部通关后展示结局海报
- [x] 墙洞图片视觉效果：色差分离+脉冲拉伸+扫描线（hover/选中触发）
- [x] 视频播放时叠加视觉效果：色差分离+脉冲拉伸+扫描线
